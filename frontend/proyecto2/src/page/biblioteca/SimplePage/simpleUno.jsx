...
export function CrearAutor() {
  const url = "http://localhost:5261/api/Contabilidad/crear-autor-biblio";  // Asegúrate de que esta URL esté correcta.
  ...
  const Modificar = async () => {
    ...
    const params = { codigo: codigoValue, nombre: nombreValue, apellido: apellidoValue };
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>{item.apellido}</td>
                  <td>
                    <button onClick={() => setCodigo(item.codigo)}>Editar</button>
                    <button onClick={() => EliminarBase(item.codigo)}>Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
